...
from pyspark.sql.types import StructType, StructField, StringType, DoubleType, DateType, FloatType, IntegerType
...
transaction_path="data/payments_data.csv"
accounts_path="data/accounts_data.csv"
output_dir = "output"
...
def validate_data(accounts_df,transaction_df, name="DataFrame"):
    try:
        ...
        for numeric_col in [f.name for f in accounts_df.schema.fields if isinstance(f.dataType, (DoubleType, FloatType,IntegerType))]:
...
# null_nan_report,dup_count, invalid_dates, outlier_count  = validate_data(accounts_df,transaction_df, name="DataFrame")
...
def load_data(monthly_result_df, output_dir, label="monthly_repayment_rates"):
    try:
        ...
        final_csv_path = os.path.join(output_dir, f"{label}_{timestamp}.csv")
        temp_dir = os.path.join(output_dir, "temp_csv_output")
        ...
        monthly_result_df.coalesce(1).write.csv(temp_dir, header=True, mode="overwrite")

        # Find the part file
        part_file = glob.glob(os.path.join(temp_dir, "part-*.csv"))[0]

        # Remove previous CSV files matching the same label
        for old_file in glob.glob(os.path.join(output_dir, f"{label}_*.csv")):
            os.remove(old_file)
        ...
        logger.info(f"Data written to: {final_csv_path}")
    except Exception as e:
...
def main():
    ...
    try:
        ...
        validate_data(transaction_df, accounts_df, name="Transactions")
        validate_data(accounts_df, transaction_df, name="Accounts")
